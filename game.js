class MenuScene extends Phaser.Scene {
    ...
    preload() {
        ...
        // También puedes cargar otros recursos como sonidos, sprites, etc.
    }
...
class GameScene extends Phaser.Scene {
    ...
    update() {
        ...
        if (score >= 30 && !gameOver) {
            ...
            // Estilizar el botón "Next level" similar a "Click to Restart"
            ...
            // Habilitar interactividad para el botón "Next level"
...
// Función para reiniciar las estrellas y las bombas
...
function startCountdown() {
    // Reiniciamos el tiempo restante
    timeRemaining = 30;

    // Actualizamos la pantalla del temporizador
    updateTimerDisplay();

    // Establecemos un intervalo para decrementar el tiempo restante
    countdownInterval = setInterval(function () {
        timeRemaining--;

        // Verificamos si se ha agotado el tiempo
        if (timeRemaining < 0) {
            clearInterval(countdownInterval); // Detenemos el intervalo
            if (score < 30) {
                endGame(); // Finalizamos el juego si no se ha alcanzado la puntuación
            }
        } else {
            updateTimerDisplay(); // Actualizamos la pantalla del temporizador
        }
...
class GameScene2 extends Phaser.Scene {
    ...
    create() {
        ...
        const { score, lives, timeRemaining, gameOver, player, platforms, cursors, stars, bombs, scene, countdownInterval } = this.game.history;

        // Reestableciendo el juego con las variables guardadas
        this.physics.resume(); // Reanudar la física del juego
        this.scene.stop('InstructionsSceneA'); // Detener la escena de instrucciones previa si estaba activa
        ...
        scoreText.setText("Score: " + score);
        livesText.setText("Lives: " + lives);
        timeRemaining.setText("Time remaining: " + timeRemaining);

        // Reiniciar estrellas y bombas
        restartStarsAndBombs(this);
    ...
    update() {
        ...
        if (score >= 30 && !gameOver) {
            ...
            const backToMenuButton = this.add.text(window.innerWidth / 2, window.innerHeight / 2 + 100, "Back to menu", {
                fontSize: "32px",
                fill: "#ffffff",
                stroke: "#000000",
                strokeThickness: 3,
            ...
            backToMenuButton.on("pointerdown", function () {
                // Ir a la siguiente escena del juego
